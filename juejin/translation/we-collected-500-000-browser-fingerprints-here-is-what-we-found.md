...
相机和打字机都可以很简单地通过它们的输出来识别和区分。

每台相机都会在图片上留下独特的噪声模式，每台打字机都会以特定的方式在字母周围留下墨迹。

通过观察两张照片并比较它们的噪声模式，我们可以准确地判定这是否是同一台相机所拍摄。

同样的原则也适用于浏览器。大多数浏览器都启用了 JavaScript 功能，它向外部公开了大量关于你的信息。

可以是你的屏幕尺寸、表情符号、已安装的字体、语言、时区或显卡模型。所有这些都可以从你的浏览器获得，但是你从未注意过这一点。

就每个指纹信息本身而言，它们都是微不足道的。但通过组合使用，任何人都可以准确地使用它来识别特定的浏览器。
...
如果你感兴趣的话, 你可以在 [我的网站](http://fp.virpo.sk) 上查看你的部分浏览器指纹信息。
...
如果有一天你的证件被盗，小偷试图从他的设备上登录，但是浏览器指纹发生了变化，银行或社交网络将可以检测到这种异常行为。为了防止诈骗，可能需要进一步的授权，比如短信验证码。
...
从某一个设备上收集单个指纹信息并不是很有价值。在一个网站上收集很多指纹信息也没有太大用处。
...
你的浏览器指纹很可能会被标记为唯一的。这听起来很可怕，但请耐心听我说，当我们看到指纹标记原理的全部时，可能就没那么可怕了。
...
导致这种变化的可能是浏览器自动更新、调整窗口大小、安装新字体，甚至调整夏令。所有这些都会让你的浏览器在很长一段时间内很难被唯一识别。

## 我们分析的数据

[Panopticlick](https://panopticlick.eff.org/static/browser-uniqueness.pdf) 和 [AmIUnique](https://hal.inria.fr/hal-01285470/file/beauty-sp16.pdf) 都发表了优秀的科学论文，他们分析了几十万个浏览器指纹。

我们的数据在几个关键方面有所不同。我们相信这有助于我们揭示更多关于浏览器指纹的信息。

* 我们分析了 566704 个浏览器指纹，这大约是之前最大的研究分析的指纹数量的两倍
* 我们的数据集中，65% 的设备是智能手机
* 数据包括 31 个不同的浏览器指纹特征，每个设备使用最先进的浏览器指纹脚本
...
* 74% 的桌面设备能够被唯一识别，而只有 45% 的移动用户能够被唯一识别。
* 在 iphone 上收集的浏览器指纹中，只有 33% 是唯一的。
* 另外 33% 的 iphone 几乎无法被跟踪，因为 20 部或更多的 iphone 显示了完全相同的浏览器指纹。
...
这张图表显示，19% 的 iphone 手机在一周内更换了指纹，而同期只有 3% 的 Android 手机也更换了指纹。因此，我们的数据可以表明，iphone 比 android 更难以跟踪更长的时间。

#### 最少的指纹个数

最后，我们讨论了到底需要从浏览器中收集多少特性才能可靠地识别它。

为此，我们通过 [香农信息熵](https://en.wikipedia.org/wiki/Entropy_(information_theory)) 来评估指纹信息到底有多强大。其熵值越高，识别过程越准确。

**例如，14.2 的熵意味着每 19000 个浏览器指纹中就有一个与我的指纹完全相同。将熵值增加到 16.5 意味着，每 92500 个设备中就有一个和我拥有相同的指纹。**

在我们的实验中，我们找到了最强的浏览器指纹信息的子集。
...
* 科技巨头可以追踪到你的线上活动，但还不完全准确。
* 智能手机(尤其是 iphone)比个人电脑更难追踪
* 设备的浏览器指纹变化非常频繁
* 浏览器指纹很容易获取

然而，如果你担心你的数据隐私，也有一些好消息。首先，[苹果宣布了一场针对浏览器指纹识别的保卫战](https://www.howtogeek.com/fyi/safari-battles-browser-fingerprinting-and-tracking-on-macos-mojave/)，它推出了最新的 Mac OS Mojave。其次，GDPR 认为浏览器指纹是个人数据，必须进行相应的处理。最后还有许多 [插件和浏览器扩展](https://amiunique.org/tools) 会混淆浏览器指纹脚本。

所以，我们的未来并不像看起来那么糟糕。诚然，有时你的浏览器可以被唯一地识别，但很多时候其他设备的浏览器指纹与你的完全相同，这使得你更难被跟踪。

#### 研究动机

在 [Slido](https://slido.com)，我们试图使我们的 web 应用程序的用户体验尽可能简单。当你加入我们的应用程序时，你通常不需要注册，我们希望保持这种方式。
...
浏览器指纹本身不足以作为我们进行用户的身份验证。然而，它们足够准确地把你放在一群有着类似兴趣的人当中(比如猫或汽车)。

---

这就意味着使用浏览器指纹是个理想的场景，如个性化的广告，准确性并非关键，或防止银行诈骗，有所畏惧到底是一件好事。
...
非常感谢我的主管, RNDr. Michal Forišek 博士, 本次研究中他给了我很大的帮助。

相关链接:

* [我的关于浏览器指纹的 60 多页的报告](http://virpo.sk/browser-fingerprinting-hraska-diploma-thesis.pdf)
* [http://fp.virpo.sk](http://fp.virpo.sk) — 了解你的浏览器指纹是什么
...
* [https://audiofingerprint.openwpm.com](https://audiofingerprint.openwpm.com/) — audio 在浏览器指纹中的应用
